    ...
    if prices.empty:
        ...
        for ticker, (_, alloc, *_) in PORTFOLIO.items():
            ...
            if ticker in prices.columns:
                ...
                port_series = s * w if port_series is None else port_series.add(s*w, fill_value=0)
                ret = (prices[ticker].dropna().iloc[-1] / prices[ticker].dropna().iloc[0] - 1) * 100
                port_return += ret * w
        ...
        real_return = port_return - inflation_drag

        c1,c2,c3,c4,c5,c6 = st.columns(6)
        c1.metric("Portfolio Return",   fmt(port_return), f"vs S&P {fmt(port_return-spy_ret)}")
        c2.metric("Real Return (CPI-adj)", fmt(real_return), f"inflation drag: {fmt(-inflation_drag)}")
        c3.metric("vs S&P 500",          fmt(spy_ret))
        c4.metric("vs Nasdaq-100",        fmt(qqq_ret),  fmt(port_return-qqq_ret))
        c5.metric("Gold",                 fmt(gld_ret))
        ...
        if port_series is not None:
            ...
            fig.add_trace(go.Scatter(x=cum.index,y=(cum*100).round(2),
        ...
        for bname,btk in BENCHMARKS.items():
            if btk in prices.columns:
                s=prices[btk].dropna(); cum=(s/s.iloc[0]-1)*100
                fig.add_trace(go.Scatter(x=cum.index,y=cum.round(2),
                    name=bname,line=dict(color=bench_colors.get(btk,"#888"),width=1.5,dash="dot"),
                    hovertemplate="%{y:.1f}%"))
        fig.update_layout(title="Cumulative Return",yaxis_title="Return (%)",height=400)
        st.plotly_chart(fig,use_container_width=True)

        # Holdings table with P/E and dividend yield
        ...
        for ticker,(name,alloc,cat,bench,bench_name,er) in PORTFOLIO.items():
            ...
            if ticker in prices.columns:
                ...
                if len(s)>1:
                    ...
                    rows.append({
                        "Ticker":ticker,"Name":name,"Category":cat,"Weight":f"{w}%",
                        f"Return ({period_label})":fmt(ret),
                        "vs Benchmark":fmt(alpha) if alpha is not None else "—",
                        "Div Yield":f"{dy:.1f}%" if dy else "—",
                        "Trail P/E":pe_badge(pe_d.get("trailingPE")),
                        "Fwd P/E":pe_badge(pe_d.get("forwardPE")),
                        "Exp Ratio":f"{er:.2f}%" if er else "0%",
                        "Status":"✅" if (alpha or 0)>=0 else ("⚠️" if (alpha or 0)>-alert_thresh else "🔴"),
                    })
        if rows:
            st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True,
                column_config={
                    ...
                    "Div Yield":st.column_config.TextColumn(width="small"),
            ...
            st.caption("🟢 P/E<15  🟡 15-25  🟠 25-35  🔴>35  |  Status vs per-holding benchmark")

        # Download CSV
        if rows:
            csv_df = pd.DataFrame(rows)
            st.download_button("⬇ Download Holdings CSV", csv_df.to_csv(index=False),
