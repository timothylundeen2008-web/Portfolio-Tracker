...
# ════════════════════════════════════════════════════════════
#  TAB: CANDIDATE REVIEW — every ETF, compared WITHIN its role
# ════════════════════════════════════════════════════════════
# The answer to "consider ALL the ETFs, not just Growth Satellite."
#
# The discipline that makes this safe rather than momentum-chasing is
# ROLE-BASED comparison: candidates compete only against others doing the
# same job. Comparing VOO to SCHG is a real question. Comparing VOO to TLT
# is a category error that would let a ranking quietly destroy the
# portfolio's diversification. Roles the regime classifier governs at Level
# 1 (metals, commodities, duration, cash, trend) allow substitution WITHIN
# the role but never reweighting ACROSS roles -- that stays the classifier's
# job.
with tab_cand:
    st.markdown("### 🔍 Candidate Review — all ETFs, compared within role")
    st.caption(
        "Every instrument in the universe scored on the same gates, compared "
        "only against others doing the same job. A candidate that "
        "materially outscores an incumbent in its own role is flagged as a "
        "substitution worth considering — with the reasoning shown, not just "
        "a ranking."
    )
    try:
        import candidate_universe as _cu
        import exposure as _exp

        _live_w = {t: custom_allocs.get(t, a)
                   for t, (_n, a, *_r) in PORTFOLIO.items()}

        _roles = _cu.all_roles()
        _role_pick = st.selectbox(
            "Role", _roles,
            format_func=lambda r: r.replace("_", " ").title())

        _members = _cu.by_role(_role_pick)
        _held = [t for t in _members if _live_w.get(t, 0) > 0]

        if _role_pick in _cu.REGIME_GOVERNED_ROLES:
            st.info(
                f"**{_role_pick.replace('_',' ').title()} is regime-governed.** "
                f"Substitution within this role is fine — a better duration "
                f"instrument is still duration. But the role's total WEIGHT "
                f"is set by the regime classifier at Level 1, not by ranking. "
                f"Momentum should never be allowed to reweight across roles."
            )
        ...
        for tk in _members:
            ...
            _rows.append({
                "Ticker": tk,
                "Name": m["name"],
                "Held %": round(_live_w.get(tk, 0), 1),
                "Leverage": f"{m['leverage']:.0f}x" if m["leverage"] > 1 else "—",
                "Expense": f"{m['expense']:.2f}%",
                "Incumbent": "✔" if m["core"] else "",
                "Top sector": max(m["sectors"], key=m["sectors"].get).title(),
            })
        ...
        if _levered_here:
            st.warning(
                f"⚠ **{', '.join(_levered_here)} in this role are leveraged "
                f"daily-reset products.** They are screened on the same gates "
                f"as everything else — as requested — but the instrument is "
                f"structurally different, not merely more aggressive. See the "
                f"Exposure & Exits tab: their stops are automatically tighter "
                f"and they carry a hard holding-period ceiling."
            )

        st.markdown("#### What a substitution would do to exposure")
        _c1, _c2 = st.columns(2)
        _out_tk = _c1.selectbox("Reduce / replace", ["(none)"] + _held)
        _in_tk = _c2.selectbox("In favour of",
                              ["(none)"] + [t for t in _members
                                            if t != _out_tk])
        if _out_tk != "(none)" and _in_tk != "(none)":
            _after = dict(_live_w)
            _moved = _after.get(_out_tk, 0)
            _after[_out_tk] = 0
            _after[_in_tk] = _after.get(_in_tk, 0) + _moved
            _cmp = _exp.compare(_live_w, _after)
            if _cmp.get("available"):
                st.markdown(
                    f"Moving **{_moved:.1f}%** from **{_out_tk}** to "
                    f"**{_in_tk}** changes notional exposure:")
                for _sec, _d in list(_cmp["deltas"].items())[:8]:
                    _clr = "#dc2626" if _d > 0 else "#16a34a"
                    st.markdown(
                        f"<span style='color:{_clr};'>{_sec.title()}: "
                        f"{_d:+.1f}pt</span>", unsafe_allow_html=True)
                st.caption(
                    f"Gross notional {_cmp['notional_before']:.0f}% → "
                    f"{_cmp['notional_after']:.0f}%. Green reduces "
                    f"concentration, red increases it.")
    except Exception as e:
        st.error(f"Candidate review unavailable: {e}")


# ════════════════════════════════════════════════════════════
#  TAB: EXPOSURE & EXITS
# ════════════════════════════════════════════════════════════
# Two things that belong together: what the book is ACTUALLY exposed to
# after look-through and leverage, and what would get you out of each
# position. Entries are optional; exits are not.
with tab_exp:
    st.markdown("### 🧬 Exposure & Exits")
    try:
        import exposure as _exp
        import exit_rules as _ex
        import candidate_universe as _cu

        _live_w = {t: custom_allocs.get(t, a)
                   for t, (_n, a, *_r) in PORTFOLIO.items()}
        _e = _exp.compute(_live_w)
        _exp.render(st, _e, "Current portfolio — look-through exposure")

        st.markdown("---")
        st.markdown("##### Exit rules by instrument type")
        st.caption(
            "Leverage changes the arithmetic, so it changes the stops. A 3x "
            "fund's -20% underlying move is roughly -60%, and recovering from "
            "-60% needs +150% — so leveraged positions get a tighter ATR "
            "multiple, a hard maximum-loss floor, and a holding-period "
            "ceiling that unleveraged positions do not."
        )
        _rows = []
        for tk in sorted(_live_w):
            if _live_w[tk] <= 0 or tk not in _cu.UNIVERSE:
                continue
            _lev = _cu.leverage_of(tk)
            _rows.append({
                "Ticker": tk,
                "Weight %": round(_live_w[tk], 1),
                "Leverage": f"{_lev:.0f}x" if _lev > 1 else "—",
                "ATR mult": (_ex.ATR_MULT_LEVERAGED if _lev > 1
                            else _ex.ATR_MULT_STANDARD),
                "Max loss floor": f"{(_ex.MAX_LOSS_LEVERAGED_PCT if _lev > 1 else _ex.MAX_LOSS_STANDARD_PCT):.0f}%",
                "Max hold": (f"{_ex.MAX_HOLD_DAYS_LEVERAGED}d"
                            if _lev > 1 else "—"),
            })
        if _rows:
            st.dataframe(pd.DataFrame(_rows), use_container_width=True,
        ...
        with st.expander("The four exit triggers — any one sufficient"):
            st.markdown(
                "1. **Structural stop** — trailing ATR-based, with a hard "
                "maximum-loss floor that overrides ATR when volatility "
                "explodes (which is exactly when ATR is least reliable).\n"
                "2. **Trend break** — a close below a FALLING 200-day. Below "
                "a *rising* one is a pullback and only tightens; the gold "
                "gate's own logic, applied to exits.\n"
                "3. **Profit ladder** — Level 6.5 R-multiple scaling: trim "
                "25% at 2R, 4R and 8R, leave 25% to run.\n"
                "4. **Thesis invalidation** — the regime or role rationale no "
                "longer holds. This fires **regardless of P&L**. 'It is still "
                "going up' is not a reason to keep something the framework no "
                "longer has a reason to own.\n\n"
                "Live per-position evaluation needs entry price, ATR and "
                "200-day for each holding — `exit_rules.plan()` computes the "
                "full picture once a position ledger with entry prices is "
                "wired in."
            )
    except Exception as e:
